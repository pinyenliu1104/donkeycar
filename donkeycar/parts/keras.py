...
def plot_yellow_line(img):
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    hsv = cv2.cvtColor(img, cv2.COLOR_RGB2HSV)
    yellow = cv2.bitwise_and(img, img, mask= cv2.inRange(hsv,np.array([20,43,200]),np.array([34,255,255])))
    p4_lst = []
    for i in get_fix_degree(yellow):
        try:
            p4_lst.append(np.percentile(np.array(i), 50))
        except:
            p4_lst.append(np.percentile(np.array([0]), 50))
    img = drawLine(img,p4_lst)
    return img

def get_fix_degree(img):
...
def drawLine(image,p4_lst):
    x1,y1,x2,y2 = p4_lst[0],p4_lst[1],p4_lst[2],p4_lst[3]
    m=slope(x1,y1,x2,y2)
    h,w=image.shape[:2]
    if m!='NA':
...
class Keras3D_CNN(KerasPilot):
    ...
    def x_transform(self, records: Union[TubRecord, List[TubRecord]]) -> XY:
        ...
        img_arrays = [rec.image(cached=True) for rec in records]
        # img_arrays = plot_yellow_line(img_arrays)
